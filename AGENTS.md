...
Use a repo-local Gradle home so cache writes stay inside the workspace. Prefix every invocation with `GRADLE_USER_HOME=.gradle-home`, for example `GRADLE_USER_HOME=.gradle-home ./gradlew clean build` to compile every module and run the full test matrix. Execute `GRADLE_USER_HOME=.gradle-home ./gradlew :sync-to-s3:test` for library-only checks, or `GRADLE_USER_HOME=.gradle-home ./gradlew :sync-to-s3-cli:shadowJar -x test` when iterating on the CLI binary. Install snapshot artifacts locally with `GRADLE_USER_HOME=.gradle-home ./gradlew publishToMavenLocal`, then generate a Sonatype upload bundle via `./generateMavenCentralBundle.sh` before pushing to staging.
